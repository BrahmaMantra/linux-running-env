...
int kvm(uint8_t code[], size_t code_len)
{
  ...
  ioctl(vmfd, KVM_SET_USER_MEMORY_REGION, &region);
    printf("KVM_SET_USER_MEMORY_REGION 0x%x\n", KVM_SET_USER_MEMORY_REGION); // 打印 exit_reason 的十六进制表示
  /* end of step 3 */
...
#define KVM_EXIT_XEN              34
  while (1)
  {
	...
	printf("run->exit_reason= 0x%x\n", run->exit_reason); // 打印 exit_reason 的十六进制表示
